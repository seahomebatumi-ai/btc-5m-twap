# TZ-10a — `sigma` or the link: the discriminating measurement

**Canonical filename: `CryptoTZ/TZ-10a-sigma-or-link.md`.** The Executor names the committed file
from this line. Report: `CryptoReports/TZ-10a-sigma-or-link-report.md`.

**This TZ replaces TZ-10, which is BLOCKED.** TZ-10's report stands on `main` and is not edited.
Three of the five `log_phi` self-tests TZ-10 §5.2 fixed could not be passed by any correct
implementation, and its §2 scope list omitted a file its own §5.2 required changing — System Map §7
items 33, 34 and 35. **The measurement, the sets, the discriminator and the pre-registered
prediction are carried over unchanged. Only §2's scope list and §5.2's self-tests differ**, and the
`log_phi` formula itself is byte-identical to TZ-10's.
...
**Required System Map revision: `2026-09-14-b`.** Required anchors:
...
- One new file, `research/tz10a-sigma-or-link.py`, on branch `tz-10a-sigma-or-link`.
...
- Scratch under `/root/tz10a-work/` only.
...
`research/tz10a-sigma-or-link.py`, on branch `tz-10a-sigma-or-link`, delivered as a pull request. It
...
Every scratch output goes to `/root/tz10a-work/`. The per-observation rows are written to
`/root/tz10a-work/tz10a-observations.csv` with enough columns to reconstruct any single row without
...
**The expectations below are literals the Architect computed and cross-checked at 60 decimal digits
before writing them, and they are compared against `pfair.phi` only where that function is itself
sound.** `pfair.phi` is `0.5*(1 + erf(z/sqrt(2)))`; below about `z = −4.5` it is the sum of two
...
both. "12 significant digits" is equality under `%.11e` formatting, and the relative difference is
printed beside each row so any other reading can be applied to the same numbers.
...
- Report straight to `main` at `CryptoReports/TZ-10a-sigma-or-link-report.md`.
- Code on `tz-10a-sigma-or-link`, pull request opened, **not merged** — merge is the Boss's, after
  the Architect's verdict.
- No Release. No dataset, archive or binary enters git history.
- **TZ-10's scratch.** `/root/tz10-work/probe-log-phi.py` is named by SHA-256 in a committed report,
  so it is copied to `/root/btc-forensics/tz10-work--probe-log-phi.py` first and verified equal at
  both ends. Then `/root/tz10-work/` is reclaimed, as the retention rule directs. **A single tree
  has not been tested against the permission classifier** — only a block of five was, in TZ-09. If
  the removal is refused, disclose it and leave the directory; it holds a few kilobytes and is not
  worth a hands-on action from the Boss.
- `/root/tz10a-work/` is reclaimed by the next TZ once this report is on `main`, on the same terms.
