# TZ-07 — the oracle feed's variance–time curve, and the corrected sigma

**Canonical filename:** `TZ-07-variance-time.md` — the Executor names the committed file from
this line and from no other source.

| item | value |
|---|---|
| destination | `CryptoTZ/TZ-07-variance-time.md` |
| report | `CryptoReports/TZ-07-variance-time-report.md` → straight to `main` |
| code | `research/tz07-variance-time.py` (new) → branch `tz-07-variance-time`, from `main` → PR. **Never straight to `main`.** |
...
|---|---|
| System Map revision | `2026-09-12-b` |
...
fit. **TZ-08 scores, out of sample, against the gate §7 fixes below — committed to git before the
...
- any non-additive change to `research/pfair.py`: `realised_sigma`, `far_branch`, `near_branch`,
  ...
  reproducible;
- any market statistic, any order path, any file under `engine/`, any Release upload.
...
## 7. The TZ-08 gate — fixed here, before the intervals it scores have happened
...
3. V1–V8 as counts, with the `G_RATIO` literals as written into the file;
